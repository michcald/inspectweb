...
class App_Instructor_Controller_Account extends Mvc_Controller
{
    /**
     *
     * @var Lib_Mein_Db_Pdo
    ...
    public function edit()
    {
        $session = new Lib_Mein_Session('auth');
        $uuid = $session->auth->UUID;
        $account = App_Instructor_Model_Users::getOne($uuid);
        ...
        if($form->isSubmitted() && $form->isValid())
        {
            ...
            App_Instructor_Model_Users::edit($uuid, $firstName, $lastName, $email, 100);
        ...
        $this->getView()->account = $account = App_Instructor_Model_Users::getOne($uuid);
    ...
    public function editPassword()
    {
        ...
        if($form->isSubmitted() && $form->isValid()) {
            App_Instructor_Model_Users::editPassword($uuid, $form->getValue('pass1'));
        } else {
            $this->getView()->form = $form;
        }
